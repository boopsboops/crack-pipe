...
for i in $FILES; do 
cutadapt --no-indels --error-rate 0.1 --overlap 10 --action=none -g file:temp/fastq/barcodes.fas -o temp/demultiplexed/"$i"/{name}.fastq.gz --discard-untrimmed temp/reorientated/reorientated.fastq.gz.split/"$i".fastq.gz &
done; wait
